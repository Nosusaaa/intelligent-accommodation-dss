              ...
              >
                <div className="flex flex-wrap items-start gap-3 sm:flex-nowrap">
                  ...
                  <div className="ml-auto flex shrink-0 items-start gap-3">
                    <Link
                      to={`/details/${p.id}`}
                      className="block h-20 w-28 overflow-hidden rounded-lg border border-slate-200 bg-slate-100"
                      aria-label="View listing details"
                    >
                      <img
                        src={
                          p.picture_url &&
                          String(p.picture_url).trim().startsWith('http')
                            ? p.picture_url
                            : PLACEHOLDER_IMAGE
                        }
                        alt={p.name || 'Listing image'}
